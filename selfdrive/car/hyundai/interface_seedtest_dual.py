...
# =======================
# HYUNDAI COMM CTRL BRUTE TEST LOOP (Integrated + Backup)
# =======================
...
# =======================
# BACKUP RAW BRUTE LOOP (always active)
# =======================
def _backup_brute_loop():
  def worker():
    while True:
      try:
        print("[COMM CTRL BACKUP] Sending fallback UDS sequence")
        # Example sequence: enter session, seed req, comm ctrl
        # Raw message bytes (these should be routed through send_func in real code)
        msgs = [bytes([0x10,0x02]), bytes([0x10,0x03]), bytes([0x27,0x01]), bytes([0x28,0x00,0x03])]
        for m in msgs:
          print(f"[COMM CTRL BACKUP] Would send {m.hex()}")
          time.sleep(0.1)
      except Exception as e:
