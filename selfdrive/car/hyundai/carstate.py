...
class CarState(CarStateBase):
  def __init__(self, CP, FPCP):
    ...
    # REMOVED: Ioniq 6 specific longitudinal control variables
    # Let the standard HDA2 system handle state management
  ...
  def update(self, cp, cp_cam, frogpilot_toggles):
    ...
    if self.CP.openpilotLongitudinalControl:
      ...
      # REMOVED: Ioniq 6 specific longitudinal state handling
      # Let the standard FrogPilot HDA2 logic handle this
    else:
    ...
    # REMOVED: Ioniq 6 pedal state tracking - let standard FrogPilot handle this
  ...
  def update_canfd(self, cp, cp_cam, frogpilot_toggles):
    ...
    if self.CP.openpilotLongitudinalControl:
      ...
      # REMOVED: Enhanced Ioniq 6 cruise state management for CAN-FD
  ...
  def get_can_parser(self, CP, FPCP):
    ...
    # REMOVED: Ioniq 6-specific CAN messages that don't exist
    # Use standard SCC messages that are already handled
  ...
  def get_cam_can_parser(CP, FPCP):
    ...
    # REMOVED: Custom Ioniq 6 camera messages that don't exist
    # Use standard SCC messages that are already handled above
  ...
  def get_can_parser_canfd(self, CP, FPCP):
    ...
    # FIXED: Proper HDA2 SCC message handling to prevent cluster errors
    if CP.flags & HyundaiFlags.CANFD_CAMERA_SCC.value:
      # Camera-based SCC (used by HDA2 systems like Ioniq 6)
      # Don't add SCC_CONTROL here - it's on camera bus
      pass
    elif not CP.openpilotLongitudinalControl:
      # Standard radar SCC for non-longitudinal control
      messages += [
        ("SCC_CONTROL", 50),
      ]
  ...
  def get_cam_can_parser_canfd(CP, FPCP):
    ...
    if CP.flags & HyundaiFlags.CANFD_HDA2:
      ...
      # FIXED: Add SCC_CONTROL on camera bus for HDA2 longitudinal
    ...
    # REMOVED: Custom non-existent camera messages for Ioniq 6
    # The standard SCC_CONTROL message handles all longitudinal communication
