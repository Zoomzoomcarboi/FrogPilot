...
class CarState(CarStateBase):
  def __init__(self, CP, FPCP):
    ...
    if CP.carFingerprint == CAR.HYUNDAI_IONIQ_6:
      ...
      self.longitudinal_enabled = False
  ...
  def update(self, cp, cp_cam, frogpilot_toggles):
    ...
    if self.CP.openpilotLongitudinalControl:
      ...
      # Ioniq 6 specific longitudinal state handling
      if self.CP.carFingerprint == CAR.HYUNDAI_IONIQ_6:
        # Read detailed cruise control state for enhanced control
        try:
          self.cruise_active = cp_cruise.vl["SCC11"]["MainMode_ACC"] == 1
          self.cruise_set_speed = cp_cruise.vl["SCC11"]["VSetDis"] * speed_conv
          self.acc_mode = cp_cruise.vl["SCC11"]["ACCMode"]
          self.scc_live = cp_cruise.vl["SCC11"]["SCC_Live"] == 1
          self.lead_distance = cp_cruise.vl["SCC12"]["ACC_ObjDist"]
          
          # Enhanced cruise state management
          ret.cruiseState.speed = self.cruise_set_speed
          ret.cruiseState.available = self.cruise_active
          
          # Handle gas/brake overrides for longitudinal control
          if (ret.gasPressed and not self.gas_pressed_prev) or (ret.brakePressed and not self.brake_pressed_prev):
            self.longitudinal_enabled = False
            ret.cruiseState.enabled = False
          elif self.cruise_active and not (ret.gasPressed or ret.brakePressed):
            self.longitudinal_enabled = True
            
        except KeyError:
          # Fallback if SCC messages not available
          pass
    else:
    ...
    # Store previous pedal states for Ioniq 6
    if self.CP.carFingerprint == CAR.HYUNDAI_IONIQ_6:
  ...
  def update_canfd(self, cp, cp_cam, frogpilot_toggles):
    ...
    if self.CP.openpilotLongitudinalControl:
      ...
      # Enhanced Ioniq 6 cruise state management for CAN-FD
      if self.CP.carFingerprint == CAR.HYUNDAI_IONIQ_6:
        # Store previous pedal states
        self.gas_pressed_prev = ret.gasPressed
        self.brake_pressed_prev = ret.brakePressed
        
        # Handle longitudinal overrides
        if ret.gasPressed or ret.brakePressed:
          self.longitudinal_enabled = False
        elif self.main_enabled and not (ret.gasPressed or ret.brakePressed):
          self.longitudinal_enabled = True
  ...
  def get_can_parser(self, CP, FPCP):
    ...
    # Enhanced message list for Ioniq 6 longitudinal control
    if CP.carFingerprint == CAR.HYUNDAI_IONIQ_6 and CP.openpilotLongitudinalControl:
      messages += [
        ("SCC11", 10),  # Enhanced cruise control set speed
        ("SCC12", 50),  # Enhanced longitudinal control
        ("SCC13", 50),  # Additional cruise info
        ("SCC14", 50),  # Longitudinal status
      ]
    elif not CP.openpilotLongitudinalControl and CP.carFingerprint not in CAMERA_SCC_CAR:
      messages += [
        ("SCC11", 50),
        ("SCC12", 50),
      ]
  ...
  def get_cam_can_parser(CP, FPCP):
    ...
    # Enhanced camera messages for Ioniq 6 longitudinal control
    if CP.carFingerprint == CAR.HYUNDAI_IONIQ_6 and CP.flags & HyundaiFlags.CANFD_CAMERA_SCC:
      messages += [
        ("SCC11", 10),  # Enhanced frequency for better control
        ("SCC12", 50),
        ("SCC13", 50),
      ]
  ...
  def get_can_parser_canfd(self, CP, FPCP):
    ...
    # Enhanced CAN-FD messages for Ioniq 6 longitudinal control
    if CP.carFingerprint == CAR.HYUNDAI_IONIQ_6 and CP.openpilotLongitudinalControl:
      messages += [
        ("SCC_CONTROL", 50),    # Standard SCC control
        ("IONIQ6_LONG_1", 20),  # Ioniq 6 specific longitudinal message 1
        ("IONIQ6_LONG_2", 10),  # Ioniq 6 specific longitudinal message 2
      ]
    elif not (CP.flags & HyundaiFlags.CANFD_CAMERA_SCC.value) and not CP.openpilotLongitudinalControl:
      messages += [
        ("SCC_CONTROL", 50),
      ]
  ...
  def get_cam_can_parser_canfd(CP, FPCP):
    ...
    if CP.flags & HyundaiFlags.CANFD_HDA2:
      ...
      # Enhanced camera SCC messages for Ioniq 6
      if CP.carFingerprint == CAR.HYUNDAI_IONIQ_6:
        messages += [
          ("IONIQ6_CAM_LONG", 20),  # Ioniq 6 camera longitudinal control
        ]
