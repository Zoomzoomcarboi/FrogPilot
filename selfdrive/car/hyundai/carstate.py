...
class CarState(CarStateBase):
  def __init__(self, CP, FPCP):
    ...
    # REMOVED: All Ioniq 6 specific longitudinal control variables
    # Let the standard HDA2 system handle all state management
    # No more: self.cruise_active, self.cruise_set_speed, self.longitudinal_enabled, etc.

    # ADDED: CAN health monitoring for HDA2 systems
    if (CP.carFingerprint == CAR.HYUNDAI_IONIQ_6 and 
        CP.flags & HyundaiFlags.CANFD_HDA2):
      self.can_error_counter = 0
      self.acc_faulted = False
  ...
  def update(self, cp, cp_cam, frogpilot_toggles):
    ...
    if self.CP.openpilotLongitudinalControl:
      ...
      # REMOVED: All Ioniq 6 specific longitudinal state handling
      # Let the standard FrogPilot HDA2 logic handle this properly
    else:
    ...
    # REMOVED: All Ioniq 6 pedal state tracking - let standard FrogPilot handle this
  ...
  def update_canfd(self, cp, cp_cam, frogpilot_toggles):
    ...
    # cruise state - CRITICAL FIX HERE
    # CAN FD cars enable on main button press, set available if no TCS faults preventing engagement
    ret.cruiseState.available = cp.vl["TCS"]["ACCEnable"] == 0  # Move this BEFORE the conditional
    
    if self.CP.openpilotLongitudinalControl:
      # For ADAS DRV ECU with openpilot longitudinal control - use TCS messages
      ret.cruiseState.enabled = cp.vl["TCS"]["ACC_REQ"] == 1
      ret.cruiseState.standstill = False
      
    else:
      # For stock longitudinal control - use conditional SCC_CONTROL access
      cp_cruise_info = cp_cam if self.CP.flags & HyundaiFlags.CANFD_CAMERA_SCC else cp
      
      # CRITICAL: Check if SCC_CONTROL exists before accessing it
      if "SCC_CONTROL" in cp_cruise_info.vl:
        ret.cruiseState.enabled = cp_cruise_info.vl["SCC_CONTROL"]["ACCMode"] in (1, 2)
        ret.cruiseState.standstill = cp_cruise_info.vl["SCC_CONTROL"]["CRUISE_STANDSTILL"] == 1
        ret.cruiseState.speed = cp_cruise_info.vl["SCC_CONTROL"]["VSetDis"] * speed_factor
        self.cruise_info = copy.copy(cp_cruise_info.vl["SCC_CONTROL"])
      else:
        # Fallback for ADAS DRV ECU cars without SCC_CONTROL
        ret.cruiseState.enabled = cp.vl["TCS"]["ACC_REQ"] == 1
        ret.cruiseState.standstill = False
    ...
    # ENHANCED: CAN health monitoring for HDA2 longitudinal control
    if (self.CP.carFingerprint == CAR.HYUNDAI_IONIQ_6 and 
        self.CP.openpilotLongitudinalControl and 
        self.CP.flags & HyundaiFlags.CANFD_HDA2):
      
      # Monitor for CAN errors or conflicts that could cause cluster issues
      try:
        # Check if we're receiving expected SCC messages from camera bus
        if hasattr(cp_cam, 'vl') and "SCC_CONTROL" in cp_cam.vl:
          # Messages flowing properly - reset error counter
          self.can_error_counter = max(0, self.can_error_counter - 1)
        else:
          # Missing expected messages - increment error counter
          self.can_error_counter += 1
      except:
        # Any error in message checking
        self.can_error_counter += 1
      
      # Check for ACC system faults that indicate conflicts
      if ret.accFaulted:
        self.acc_faulted = True
        self.can_error_counter += 2  # ACC faults are serious
      else:
        self.acc_faulted = False
  ...
  def get_can_parser(self, CP, FPCP):
    ...
    # REMOVED: All Ioniq 6-specific CAN messages that don't exist in DBC
    # Use only standard SCC messages that are properly defined
  ...
  def get_cam_can_parser(CP, FPCP):
    ...
    # REMOVED: All custom Ioniq 6 camera messages that don't exist
    # Use only standard SCC messages that are already handled above
  ...
  def get_can_parser_canfd(self, CP, FPCP):
    ...
    # CRITICAL FIX: Proper HDA2 SCC message routing to prevent cluster errors
    if CP.flags & HyundaiFlags.CANFD_HDA2:
      # For HDA2 systems, SCC messages are on camera bus, not ECAN
      # Don't add SCC_CONTROL here - it will be added to camera parser
      pass
    elif CP.flags & HyundaiFlags.CANFD_CAMERA_SCC.value:
      # Camera-based SCC but not HDA2 - don't add here either
      pass  
    elif not CP.openpilotLongitudinalControl:
      # Standard radar SCC for non-longitudinal control only
  ...
  def get_cam_can_parser_canfd(CP, FPCP):
    ...
    if CP.flags & HyundaiFlags.CANFD_HDA2:
      ...
      # CRITICAL: For HDA2, SCC_CONTROL messages are on camera bus
      # Add them regardless of longitudinal control mode to prevent KeyError
      messages += [("SCC_CONTROL", 50)]
        
    elif CP.flags & HyundaiFlags.CANFD_CAMERA_SCC:
      # Standard camera SCC (non-HDA2)
      messages += [("SCC_CONTROL", 50)]
