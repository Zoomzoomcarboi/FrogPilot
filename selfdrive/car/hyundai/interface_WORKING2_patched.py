...
class UDSTester:
  def __init__(self, logcan, sendcan, CP=None):
    ...
    self.bus = getattr(CanBus(CP), "ECAN", 0) if CP is not None else 0
...
def disable_ioniq6_adas_ecu_comprehensive(logcan, sendcan, CP):
  ...
  try:
    ...
    # 1) Tester Present
    disable_ecu(logcan, sendcan, bus=bus, addr=req_id, com_cont_req=bytes([0x3E, 0x00]))

    # 2) Diagnostic Session Control: Extended
    disable_ecu(logcan, sendcan, bus=bus, addr=req_id, com_cont_req=bytes([0x10, 0x03]))

    # 3) SecurityAccess seed request (0x27 0x01)
    print("[HYUNDAI] Requesting SecurityAccess seed (0x27 0x01)...")
    disable_ecu(logcan, sendcan, bus=bus, addr=req_id, com_cont_req=bytes([0x27, 0x01]))
    time.sleep(1.0)  # Give time for seed response

    # 4) Communication Control: disable Tx/Rx (0x28 0x00 0x03)
    disable_ecu(logcan, sendcan, bus=bus, addr=req_id, com_cont_req=bytes([0x28, 0x00, 0x03]))
...
class CarInterface(CarInterfaceBase):
  ...
  def init(CP, logcan, sendcan):
    global uds_tester
    try:
        print(\"[HYUNDAI][UDS] Starting disable sequence...\")
        uds_tester = UDSTester(logcan, sendcan, CP)
        uds_tester.start()
    except Exception as e:
        print(f\"[HYUNDAI][UDS] Failed to start UDSTester: {e}\")
