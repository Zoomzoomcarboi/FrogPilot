...
class CarInterface(CarInterfaceBase):
  ...
  def _get_params(ret, candidate, fingerprint, car_fw, experimental_long, docs, frogpilot_toggles):
    ...
    if candidate in CANFD_CAR:
      ...
      # Enable longitudinal control for Ioniq 6 specifically, exclude others as before
      if candidate in IONIQ_6_LONGITUDINAL_CAR:
        ret.experimentalLongitudinalAvailable = True
        # Special tuning for Ioniq 6
        if candidate == CAR.HYUNDAI_IONIQ_6:
          ret.longitudinalTuning.kpV = [0.15, 0.3, 0.5]  # Conservative tuning
  ...
  def _update(self, c, frogpilot_toggles):
    ...
    # Special handling for Ioniq 6 longitudinal control
    if self.CS.CP.carFingerprint == CAR.HYUNDAI_IONIQ_6 and self.CS.CP.openpilotLongitudinalControl:
      # Override cruise state if using openpilot longitudinal
      if ret.cruiseState.enabled and not self.CS.main_enabled:
        ret.cruiseState.enabled = False
        ret.cruiseState.speed = 0
        ret.cruiseState.speedOffset = 0
        
      # Handle gas/brake pedal overrides
