...
# --- BEGIN: surgical UDS send/log patch (non-destructive) ---
try:
  from openpilot.selfdrive.car.disable_ecu import disable_ecu as _original_disable_ecu
  ...
  def disable_ecu(logcan, sendcan, *, bus, addr, com_cont_req=b"\x28\x83\x01"):
    """Wrapper to normalize payload types and safely log UDS responses."""
    # 1) normalize payload to bytes
    com_cont_req = _to_bytes(com_cont_req)

    # log what we send
    try:
      print(f"[HYUNDAI][UDS] Sent to 0x{addr:X}: {com_cont_req.hex()}")
    except Exception:
      print(f"[HYUNDAI][UDS] Sent to 0x{addr:X}: <non-hex encodable>")

    # call the original function
    ret = _original_disable_ecu(logcan, sendcan, bus=bus, addr=addr, com_cont_req=com_cont_req)

    # 2) brief, bounded listener for UDS-ish responses (≤1.0s, non-blocking boot)
    try:
      ...
      while time.time() - start < 1.0:
        ...
        if msg is None:
          continue
        for m in msg.can:
          if m.src == bus and 0x700 <= m.address <= 0x7FF:
            try:
              resp_bytes = bytes(m.dat) if hasattr(m, "dat") else (bytes(m) if isinstance(m, (bytes, bytearray)) else b"")
            except Exception:
              resp_bytes = b""
            if resp_bytes:
              try:
                hex_resp = resp_bytes.hex()
              except Exception:
                hex_resp = "<non-hex encodable>"
              print(f"[HYUNDAI][UDS] Response from 0x{m.address:X}: {hex_resp}")
              if len(resp_bytes) >= 2:
                sid = resp_bytes[0]
                subf = resp_bytes[1]
                if sid == 0x68:
                  print("[HYUNDAI][UDS] COMM CONTROL SUCCESS")
                elif sid == 0x7F:
                  nrc = resp_bytes[2] if len(resp_bytes) > 2 else 0x00
                  print(f"[HYUNDAI][UDS] NEGATIVE: Service=0x{subf:02X}, NRC=0x{nrc:02X}")
              raise StopIteration
  ...
  # patch into module namespace so existing imports use our wrapper
  import openpilot.selfdrive.car.disable_ecu as _decumod
  _decumod.disable_ecu = disable_ecu
  print("[HYUNDAI] Patched disable_ecu with safe logging")
except Exception as _e:
  print(f"[HYUNDAI] disable_ecu patch skipped: {_e}")
# --- END: surgical UDS send/log patch ---
