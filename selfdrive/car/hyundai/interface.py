...
# === Ioniq 6: ADAS Disable (with SecurityAccess seed logging) + SCC_CONTROL debug + stock monitor ===
...
def disable_ioniq6_adas_ecu_comprehensive(logcan, sendcan, CP):
  ...
  Adds a SecurityAccess (0x27 0x01) seed request and logs the ECU's response seed.
  Also keeps the original tester present + extended session + comm control steps.
  """
  ...
  try:
    ...
    # 3) SecurityAccess seed request (0x27 0x01) and capture response on 0x7D8
    try:
      print("[HYUNDAI] Requesting SecurityAccess seed (0x27 0x01)…")
      _log_security_seed_once(logcan, bus=bus, resp_addr=resp_id, window_s=2.0)
      disable_ecu(logcan, sendcan, bus=bus, addr=req_id, com_cont_req=[0x27, 0x01])
    except Exception as e:
      print(f"[HYUNDAI] SecurityAccess seed request send failed: {e}")

    # 4) Communication Control: disable Tx/Rx (likely ignored without key; logged for visibility)
    disable_ecu(logcan, sendcan, bus=bus, addr=req_id, com_cont_req=[0x28, 0x00, 0x03])
    print("[HYUNDAI] ADAS disable sequence sent (note: 0x28 may be ignored without security key)")
...
def _log_security_seed_once(logcan, *, bus: int, resp_addr: int, window_s: float = 2.0):
  """
  Listens to the 'can' sub socket for up to window_s seconds
  and logs the first UDS security access response from resp_addr.
  """
  try:
...
try:
  ...
  def _monitor_stock_scc():
    ...
    seen_stock = False
    start = time.time()
    stock_addrs = {0x420, 0x421, 0x50, 0x51}  # common SCC_CONTROL addresses, varies by platform
