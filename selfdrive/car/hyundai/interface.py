...
# =======================
# HYUNDAI COMM CTRL BRUTE TEST LOOP (Response-aware)
# =======================
...
class HyundaiCommCtrlTester:
    ...
    while not self.stop:
      for sess in sessions:
        ...
        try:
          ...
          print("[COMM CTRL TEST] Error sending session:", e)
        for ct in ctrlTypes:
          for cm in commTypes:
            if self.stop: break
            try:
  ...
  def handle_response(self, resp_bytes):
    # Called from UDS response handler
    if self.stop:
      return
    if len(resp_bytes) > 0 and resp_bytes[0] == 0x68:
      self.stop = True
      print(f"[COMM CTRL TEST] SUCCESS with response {resp_bytes.hex()}")

# Example usage:
# tester = HyundaiCommCtrlTester(send_func)
# tester.handle_response(resp)  # call when UDS response received
