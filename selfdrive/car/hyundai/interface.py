...
class CarInterface(CarInterfaceBase):
  ...
  def _get_params(ret, candidate, fingerprint, car_fw, experimental_long, docs, frogpilot_toggles):
    ...
    if candidate in CANFD_CAR:
      ...
      # MODIFIED: Enable Ioniq 6 longitudinal control
      ret.experimentalLongitudinalAvailable = (candidate not in (CANFD_UNSUPPORTED_LONGITUDINAL_CAR | CANFD_RADAR_SCC_CAR)) or \
                                             candidate == CAR.IONIQ_6
    else:
      if use_old_long:
        ret.longitudinalTuning.deadzoneBP = [0.]
        ret.longitudinalTuning.deadzoneV = [0.]
    ...
    ret.startAccel = 1.0
    if candidate != CAR.IONIQ_6:  # Use default for non-Ioniq 6
