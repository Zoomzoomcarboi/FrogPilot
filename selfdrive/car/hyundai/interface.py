...
# === Ioniq 6 ADAS Disable and SCC_CONTROL Debug Patch ===
_ADAS_DISABLE_ATTEMPTED = False

def disable_ioniq6_adas_ecu_comprehensive(logcan, sendcan, CP):
  """
  Force disable the camera ECU on Ioniq 6 so OP longitudinal can take over.
  Always runs once per boot (no env var needed).
  """
  ...
  try:
    ...
    # 1. Tester Present
    disable_ecu(logcan, sendcan, bus=bus, addr=addr, com_cont_req=[0x3E, 0x00])
    # 2. Diagnostic Session Control (Extended)
    disable_ecu(logcan, sendcan, bus=bus, addr=addr, com_cont_req=[0x10, 0x03])
    # 3. Communication Control (disable Tx/Rx)
    disable_ecu(logcan, sendcan, bus=bus, addr=addr, com_cont_req=[0x28, 0x00, 0x03])

    print("[HYUNDAI] ADAS disable sequence sent to camera ECU")
  except Exception as e:
...
try:
  ...
  def _monitor_stock_scc():
    ...
    seen_stock = False
    start = time.time()
    while time.time() - start < 15:  # watch first 15s after boot
      canmsg = sub.recv()
      for m in canmsg.can:
        if m.address in (0x420, 0x421, 0x50, 0x51):  # common SCC_CONTROL addrs
          if not seen_stock:
