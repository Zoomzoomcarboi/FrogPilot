...
def disable_ioniq6_adas_ecu_comprehensive(logcan, sendcan, CP):
  ...
  for seq, desc in alt_sequences:
    try:
      disable_ecu(logcan, sendcan, bus=bus, addr=addr, com_cont_req=seq)
...
# =======================
# HYUNDAI COMM CTRL BRUTE TEST (Integrated + Active Backup, FULL MATRIX)
# =======================
import threading, time, os

# Global plumbing so backup can send once HyundaiInterface is ready
_HY_COMMCTRL_SEND = {"func": None}
_HY_BACKUP_STARTED = {"flag": False}

class HyundaiCommCtrlTester:
  def __init__(self, send_func):
    self.send_func = send_func
    self.stop = False
    self.seed_log_path = "/data/openpilot/ioniq6_seeds.log"
    print("[HYUNDAI TEST] HyundaiCommCtrlTester started")
  ...
  def log_seed(self, seed_bytes):
    try:
      ...
      print(f"[SECURITY ACCESS] Seed logged: {seed_bytes.hex()}")
    except Exception as e:
      print("[SECURITY ACCESS] Failed to log seed:", e)

  def worker(self):
    sessions = [0x02, 0x03]              # programming, extended
    ctrlTypes = [0x00,0x01,0x02,0x03]    # enable, rx-off, tx-off, rx+tx off
    commTypes = [0x01,0x02,0x03]         # normal, NM, both
    while not self.stop:
      for sess in sessions:
        ...
        try:
          ...
          msg = bytes([0x10, sess])
          self.send_func(msg)
          print(f"[COMM CTRL TEST][Session=0x10 {sess:02X}] Sent {msg.hex()}")
          time.sleep(0.15)
          # request seed
          seed_req = bytes([0x27, 0x01])
          self.send_func(seed_req)
          print("[SECURITY ACCESS] Sent 2701")
          time.sleep(0.15)
        except Exception as e:
          print("[COMM CTRL TEST] Error sending session/seed:", e)
        for ct in ctrlTypes:
          for cm in commTypes:
            if self.stop: break
            try:
              ...
              print("[COMM CTRL TEST] Error sending comm ctrl:", e)
      time.sleep(10)
  ...
  def handle_response(self, resp_bytes):
    if self.stop:
      return
    if len(resp_bytes) >= 2:
      # Positive SecurityAccess seed response
      if resp_bytes[0] == 0x67 and resp_bytes[1] == 0x01:
        seed = resp_bytes[2:]
        print(f"[SECURITY ACCESS] Response: {resp_bytes.hex()} (seed={seed.hex()})")
        self.log_seed(seed)
      # Positive CommunicationControl
      elif resp_bytes[0] == 0x68:
        self.stop = True
        print(f"[COMM CTRL TEST] SUCCESS with response {resp_bytes.hex()}")
      # Negative response
      elif resp_bytes[0] == 0x7F:
        print(f"[COMM CTRL TEST] Negative Response: {resp_bytes.hex()}")


# Hook into HyundaiInterface
try:
...
if _HyundaiInterface is not None:
  class HyundaiInterface(_HyundaiInterface):
    def __init__(self, CP, CarController, CarState):
      ...
      try:
        # expose send function for backup loop
        _HY_COMMCTRL_SEND["func"] = self.can_uds_request
        self.commtester = HyundaiCommCtrlTester(self.can_uds_request)
      except Exception as e:
        print("[COMM CTRL TEST] Failed to start tester:", e)
      # ensure backup loop starts once
      try:
        if not _HY_BACKUP_STARTED["flag"]:
          _start_backup_brute_loop()
          _HY_BACKUP_STARTED["flag"] = True
      except Exception as e:
        print("[COMM CTRL BACKUP] Could not start backup:", e)

    def handle_can_response(self, resp):
      try:
        if hasattr(self, "commtester"):
          self.commtester.handle_response(resp.dat)
      except Exception as e:
        print("[COMM CTRL TEST] Error handling response:", e)
      return super().handle_can_response(resp)


# =======================
# ACTIVE BACKUP BRUTE LOOP (always available, FULL MATRIX)
# =======================
def _start_backup_brute_loop():
  def worker():
    # Wait until send function is available
    while _HY_COMMCTRL_SEND["func"] is None:
      print("[COMM CTRL BACKUP] Waiting for send function...")
      time.sleep(1)
    send = _HY_COMMCTRL_SEND["func"]
    sessions = [0x02, 0x03]
    ctrlTypes = [0x00,0x01,0x02,0x03]
    commTypes = [0x01,0x02,0x03]
    while True:
      try:
        print("[COMM CTRL BACKUP] Running fallback UDS matrix")
        for sess in sessions:
          # session
          msg = bytes([0x10, sess])
          send(msg)
          print(f"[COMM CTRL BACKUP] Sent {msg.hex()}")
          time.sleep(0.2)
          # seed
          seed_req = bytes([0x27, 0x01])
          send(seed_req)
          print("[COMM CTRL BACKUP] Sent 2701")
          time.sleep(0.2)
          # full matrix of 0x28
          for ct in ctrlTypes:
            for cm in commTypes:
              m = bytes([0x28, ct, cm])
              send(m)
              print(f"[COMM CTRL BACKUP] Sent {m.hex()}")
              time.sleep(0.2)
      except Exception as e:
        print("[COMM CTRL BACKUP] Error:", e)
      time.sleep(30)
  threading.Thread(target=worker, daemon=True).start()

# Kick backup at import (it will wait for send function)
if not _HY_BACKUP_STARTED["flag"]:
  try:
    _start_backup_brute_loop()
    _HY_BACKUP_STARTED["flag"] = True
  except Exception as e:
    print("[COMM CTRL BACKUP] Failed to start at import:", e)
