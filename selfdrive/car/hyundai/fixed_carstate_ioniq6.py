...
class CarState(CarStateBase):
  ...
  def get_can_parser(self, CP, FPCP):
    ...
    # FIXED: Use only existing DBC messages for Ioniq 6
    if CP.carFingerprint == CAR.HYUNDAI_IONIQ_6 and CP.openpilotLongitudinalControl:
      messages += [
        ("SCC11", 10),  # Use existing SCC messages
        ("SCC12", 50),  # Use existing SCC messages
        # REMOVED non-existent messages: SCC13, SCC14
      ]
  ...
  def get_cam_can_parser(CP, FPCP):
    ...
    # FIXED: Use only existing SCC messages for Ioniq 6
    if CP.carFingerprint == CAR.HYUNDAI_IONIQ_6 and CP.flags & HyundaiFlags.CANFD_CAMERA_SCC:
      messages += [
        ("SCC11", 10),  # Use existing messages only
        ("SCC12", 50),
        # REMOVED: ("SCC13", 50) - doesn't exist
        # REMOVED: ("IONIQ6_CAM_LONG", 20) - doesn't exist
      ]
  ...
  def get_can_parser_canfd(self, CP, FPCP):
    ...
    # FIXED: Remove non-existent messages for Ioniq 6
    # Use standard SCC_CONTROL message that exists in DBC
  ...
  def get_cam_can_parser_canfd(CP, FPCP):
    ...
    if CP.flags & HyundaiFlags.CANFD_HDA2:
      ...
      # REMOVED: Custom camera message that doesn't exist
      # ("IONIQ6_CAM_LONG", 20) - doesn't exist in DBC
